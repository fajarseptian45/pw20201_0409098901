...
class Auth extends CI_Controller
{
    ...
    public function registration()
    {
        ...
        if ($this->form_validation->run() == false) {
            $data['title'] = 'SIDOKAR Registration';
            $this->load->view('templates/auth_header', $data);
            ...
            Congratulation! Your account has been created. Please activate your account in 24 hours.</div>');
            redirect('auth');
        }
    }
    ...
    private function _sendEmail($token, $type)
    {
        $config = [
            ...
            'smtp_user' => '', //email smtp kalian
            'smtp_pass' => '', //password smtp mail
            'smtp_port' => 465,
            'mailtype'  => 'html',
            'charset'   => 'utf-8',
            'newline'   => "\r\n"
        ...
        $this->email->from('sidakpus@gmail.com', 'Admin SIDAKPUS');
        $this->email->to($this->input->post('email'));

        if ($type == 'verify') {
            $this->email->subject('Account Verification SIDAKPUS');
            ...
            $this->email->subject('Reset Password SIDAKPUS');
        ...
        if ($user) {
            ...
            if ($user_token) {
                if (time() - $user_token['date_created'] < (60 * 60 * 24)) {
                    ...
                    $this->session->set_flashdata('message', '<div class="alert alert-success" role="alert">
                    Congratulation! ' . $email . ' has been activated. Please login.</div>');
                    redirect('auth');
                } else {
    ...
    public function logout()
    {
        ...
        $this->session->set_flashdata('message', '<div class="alert alert-success" role="alert">
            You have been logged out. Thank you!</div>');
        redirect('auth');
    }
